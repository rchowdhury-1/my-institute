...
export default function MessagingPage({ authChecked, logTag, roleLabel }: MessagingPageProps) {
  ...
  useEffect(() => {
    ...
    api.get("/messages/conversations")
      ...
      .catch((err) => console.error(`[${logTag}] failed to load conversations:`, err))
  ...
  function displayRole(role: string) {
    return roleLabel ? (roleLabel[role] ?? role) : role;
  }
            ...
            {conversations.length === 0 ? (
              ...
              conversations.map((conv) => (
                ...
                >
                  ...
                  <p className={`text-xs text-charcoal/40${roleLabel ? "" : " capitalize"}`}>{displayRole(conv.other_role)}</p>
                  {conv.last_message && (
            ...
            {!activeConv ? (
              ...
              <>
                <div className="px-5 py-4 border-b border-black/5">
                  ...
                  <p className={`text-xs text-charcoal/40${roleLabel ? "" : " capitalize"}`}>{displayRole(activeConv.other_role)}</p>
                </div>
